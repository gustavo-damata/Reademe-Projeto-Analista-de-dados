...
-- Qual o departamento com o maior número de pessoas?

SELECT d.Dname AS 'Departamento', COUNT(*) AS 'Número de Pessoas'
FROM employee e
JOIN departament d ON e.Dept_number = d.Dnumber
GROUP BY d.Dname
ORDER BY COUNT(*) DESC
LIMIT 1;
...
SELECT d.Dname AS 'Departamento', e.Cidade
FROM employee e
JOIN departament d ON e.Dept_number = d.Dnumber
ORDER BY e.Cidade, d.Dname;

-- Relação de empregados por departamento

SELECT Dname AS 'Departamento', COUNT(*) AS 'Número de Empregados'
FROM employee e
JOIN departament d ON e.Departamento = d.Dnumber
GROUP BY Dname
ORDER BY Dname;
...
FOR EACH ROW 
BEGIN
select employee from ssn where idssn = New.super_Ssn;
END //
...
CREATE TRIGGER before_delete_dependent
BEFORE UPDATE ON dependent
FOR EACH ROW 
BEGIN
   select dependent from employee where idemployee = old.dependent;
